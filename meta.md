...
- 标准风格（默认）：Markdown 行号链接 `[文件名 (line N)](/绝对路径:N)`，与新版 Codex 渲染出的行号链接格式一致，多文件逗号分隔
...
- **Coding Rules**:
  ...
  - 标准风格链接目标使用文件绝对路径（`uri.fsPath`）；简化风格经 `workspace.asRelativePath` 转为工作区相对路径
...
| 场景 | 标准风格 | 简化风格 |
| --- | --- | --- |
| 整文件 | `[foo.ts](/abs/path)` | `[@src/foo.ts]` |
| 单行 | `[foo.ts (line 10)](/abs/path:10)` | `[@src/foo.ts#10]` |
| 多行 | `[foo.ts (line 10-20)](/abs/path:10-20)` | `[@src/foo.ts#10-20]` |
| 多文件 | `[a.ts](/abs/a.ts), [b.ts](/abs/b.ts)` | `[@src/a.ts][@src/b.ts]` |

- **关键函数**:
  - `getReferenceStyle()` - 读取当前引用样式
  - `fileBaseName()` / `linkTarget()` - 标准风格标签（文件名）与链接目标（绝对路径，含空格/括号时用 `<>` 包裹）
